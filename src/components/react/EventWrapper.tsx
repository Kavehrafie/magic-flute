...
export default function EventWrapper({ children, title, locale = DEFAULT_LOCALE, links }: EventWrapperProps) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{useTranslations(locale)('form.event-subscription.email')}</FormLabel>
                  <FormControl>
                    <Input placeholder="email@dot.com" {...field} />
                  </FormControl>
                  <FormDescription>
                    {useTranslations(locale)('form.event-subscription.description')}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
